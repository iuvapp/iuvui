...
Pro sells maintained design outcomes and workflow leverage:

- advanced Style Packs and premium visual variants;
- managed source downloads and upgrades through `@iuvui/cli`;
- the iuvui MCP service;
...
Free users may always write their own CSS because iuvui runs in their application. The commercial boundary is therefore a product and distribution boundary, not an artificial technical restriction: iuvui guarantees and documents base styles, base variants, and basic token customization for free, while curated advanced recipes, premium visual variants, and their ongoing maintenance are Pro products.
...
Production applications remain available when iuvui services are offline. There is no browser call-home, secret in the frontend bundle, or runtime license gate. Previously licensed builds continue to run after a subscription ends; access to new downloads, updates, MCP, and managed services stops according to the commercial terms.
...
`@iuvui/cli` remains publicly installable so every user can run help, diagnostics, authentication, and project inspection. Commands that download managed component source, advanced styles, or Pro migrations require an entitlement.

The MCP server uses the same entitlement service and artifact catalog as the CLI. It may recommend components for free, while Pro actions such as retrieving maintained source, applying a Style Pack, generating licensed compositions, or planning an authenticated upgrade require the appropriate scope.
